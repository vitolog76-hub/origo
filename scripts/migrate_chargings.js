/*
Migration script for chargings collection.
Usage:
  - Place service account JSON path in env var GOOGLE_APPLICATION_CREDENTIALS, or pass path as first arg.
  - Run in dry-run (no writes):
      node scripts/migrate_chargings.js --dry
  - Run to commit changes:
      node scripts/migrate_chargings.js --commit

What it does:
  - Reads all documents in `chargings`.
  - Groups by userId + year-month.
  - For each group, reads user profile to get providerVariableRate and providerFixedMonthlyFee.
  - Computes areraCost, providerVariableCost, providerFixedPortion and new totalCost for each doc.
  - Performs batched updates (500 ops per batch, commits multiple batches if needed).
*/
